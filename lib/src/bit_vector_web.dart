...
class BitVector {
  ...
  @override
  String toString() =>
      _words.map<String>((i64) => i64.toHexString()).toString();

  List<String> toListforStorage() =>
      _words.map<String>((Int64 i) => i.toString()).toList();

  // static BitVector fromString(String inStr) =>
  //     BitVector.fromWords(new Int64List.fromList(inStr));

  /// Creates a new [BitVector] instance that is equal to [other].
  BitVector.from(BitVector other)
      : _words = List<Int64>.from(other._words),
        _wordsInUse = other._wordsInUse;

  /// Creates a new [BitVector] instance that is equal to [other].
  BitVector.fromWordsString(List<String> words)
      : _words = List<Int64>.from([]) {
    //(words.map<Int64>((String w) => int64).toList()) {
    _recalculateWordsInUse();
  }

  /// Creates a new [BitVector] instance that is equal to [other].
  BitVector.fromWords(List<Int64> words) : _words = words {
    _recalculateWordsInUse();
  }
  ...
  int get length {
    ...
    return _bitsPerWord * (_wordsInUse - 1) +
        (_bitsPerWord -
            new Int64(_words[_wordsInUse - 1].toInt()).numberOfLeadingZeros());
  }

  // removed due to lack of support in JavaScript
  ...
  void _initWord(int length) {
    _words =
        new List<Int64>.generate(_wordIndex(length - 1) + 1, (i) => Int64(0));
  }
  ...
  void set(int bitIndex) {
    ...
    _expandTo(wordIndex);
    final before = _words[_wordIndex(bitIndex)];

    _words[wordIndex] |= (1 << (bitIndex & _bitMask));
    print(
        " word: ${_wordIndex(bitIndex)} bit: ${bitIndex} before; $before after: ${_words[_wordIndex(bitIndex)]}"
        " (1 << ($bitIndex & $_bitMask))");
  }
  ...
  void _ensureCapacity(int wordsRequired) {
    if (_words.length < wordsRequired) {
      ...
      _words = new List<Int64>.generate(request, (i) => Int64(0))
        ..setRange(0, _words.length, _words);
    }
  }
}
